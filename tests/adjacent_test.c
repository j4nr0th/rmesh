...
int main(void)
{
    ...
    enum {ORDER = 3};
    geo_id* adjacent = calloc((2 * ORDER + 1) * (2 * ORDER + 1), sizeof(*adjacent));
    assert(adjacent);
    memset(adjacent, ~0, (2 * ORDER + 1) * (2 * ORDER + 1) * sizeof(*adjacent));

    e = surface_centered_element(&m, 1, ORDER, adjacent);
    assert(e == MESH_SUCCESS);
    if (e != MESH_SUCCESS)
    {
        exit(EXIT_FAILURE);
    ...
    for (unsigned i = 0; i < 2 * ORDER + 1; ++i)
    {
        ...
        {
            printf("%3d ", adjacent[i * (2 * ORDER + 1) + j]);
        }
        printf("\n");
    }
    free(adjacent);
